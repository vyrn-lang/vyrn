...
const PASSING: &[&str] = &[
    ...
    "codecbytes.vyrn",
    // RFC-0077 M2m: `spawn`/`join` (RFC-0025). On wasm there are no threads, so
    // the shim's own `__vyrn_spawn` calls the thunk inline — this backend emits
    // that eager call directly and therefore never forms a function pointer.
    // `concurrency` is the fork-join surface with a `share`d aggregate argument;
    // `parallel` (below) joins four tasks twice each, which is the idempotence.
    "concurrency.vyrn",
    "consume.vyrn",
    "domdemo.vyrn",
    "ecs.vyrn",
