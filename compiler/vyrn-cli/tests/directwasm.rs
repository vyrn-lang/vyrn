...
const PASSING: &[&str] = &[
    ...
    "lambdas.vyrn",
    "linkedlist.vyrn",
    // RFC-0077 M2n: RFC-0008's leveled logger. The threshold is folded here as it
    // is everywhere else — `logging { level: debug }` means this file's `.trace(..)`
    // emits no write, and `[TRACE] ` is not in the module at all.
    "logging.vyrn",
    "map.vyrn",
    "mapdemo.vyrn",
    "modify.vyrn",
    ...
    "parallel.vyrn",
    "region.vyrn",
    // Passing since `spawn` and `region` landed, and never listed — the ladder
    // asked for it on every run. Not this milestone's work; locked in here so a
    // regression is a failure rather than a missing line.
    "controlflow.vyrn",
];
