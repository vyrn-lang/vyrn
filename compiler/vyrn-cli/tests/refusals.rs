...
//! 28, 06, 20, 21, 07, 19, 25, 13, 14, 01, 02, 03, 27 and 34, RFC-0125 §3 M3 —
//! is refused by the kernel in both runs, and the
...
fn the_shapes_rule_twos_unit_tests_pinned_are_still_refused() {
    ...
    let cases: &[(&str, Vec<&str>, Vec<&str>, String)] = &[
        ...
        (
            "a record literals field",
            vec![
                "`x` may not be stored into the literal — it is a `read` parameter",
...
fn sections() -> Vec<Section> {
    ...
    vec![
        ...
        sec(
            "enum TakeForm {",
            Checker,
            "which form wrote the `consume`, and how a refusal names it — read \
             by `check_take` alone",
        ),
        sec(
        ...
        sec(
            "    fn check_take(",
            Checker,
            "a take's two refusals, module state and a borrow, in three forms \
             (rows 10, 11, 29). The kernel refuses all three and names the \
             wrong form: a prefix `consume` is worded as the call it feeds, \
             and a `for .. in consume` as the `drop` its lowering places",
        ),
        sec(
