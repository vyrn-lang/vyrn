...
fn run() {
    ...
    assert_eq!(
        ...
        554,
        "the values the declared reading could not name, which nobody freed          (548 at the argument slice, plus four in `tallybytes.vyrn` and          `falliblegeneric.vyrn`, and two in `looptemp.vyrn`, which joined the          corpus beside it)"
    );
    assert_eq!(
