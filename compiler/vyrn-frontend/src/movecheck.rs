...
mod tests {
    ...
    fn a_field_read_does_not_take_the_record() {
        // A place owns its contents (rule 4), so `r.s` is a projection: naming
        // it locally is free. Returning that name is rule 3, which the kernel
        // states now, and the shape is pinned in `vyrn-cli/tests/refusals.rs`
    ...
    /// RFC-0089 rule 3, Phase 6, from the other side: the two shapes module
    /// state's return rule does NOT refuse.
    ///
    /// The refusals moved to `vyrn-cli/tests/refusals.rs` with the rule
    /// (RFC-0125 §3 M3, row 17). These stay, because each is an acceptance and
    /// this pass still decides both: a copy is a value of its own, and a type
    /// nobody releases carries nothing out.
    #[test]
    fn returning_module_state_is_refused() {
        assert!(run(
            "let mut title = \"x\" fn get() -> String { return title.copy() }                      fn main() -> Int64 { return 0 }"
        )
        .is_ok());
        assert!(run("type R = { n: Int64 } let mut r = R { n: 1 }                      fn get() -> R { return r } fn main() -> Int64 { return 0 }")
        .is_ok());
    }

    /// The fix an arm-yielded projection is offered, either side of the export
    /// boundary: it compiles.
    ///
    /// The refusals are the kernel's since RFC-0125 §3 M3, row 17, and both
    /// spellings are pinned in `vyrn-cli/tests/refusals.rs`. What is asked here
    /// is what this pass still answers: `s.copy()` is a value of its own, so
    /// nothing about it is a lend.
    #[test]
    fn an_export_may_not_lend_its_result() {
        let enum_and_state = "type Tag = | Word(String) | Num(Int64)                               let mut tag = Word(\"w\") ";
        let fixed = "return match tag { Word(s) => s.copy(), Num(n) => \"num\", } }                      fn main() -> Int64 { return 0 }";
    ...
    /// The fix an export's store is offered: `arg.copy()`, and it compiles.
    ///
    /// The refusal and its menu are the kernel's since RFC-0125 §3 M3, rows 01,
    /// 02, 03, 27 and 34, and `vyrn-cli/tests/refusals.rs` asks both of the
    /// whole compiler. What is left here is the way out.
    #[test]
    fn an_exports_store_takes_a_copy() {
        assert!(run("let mut kept = \"x\"                      export extern fn set(arg: String) { kept = arg.copy() }                      fn main() -> Int64 { return 0 }")
        .is_ok());
    }

    /// The two ways out of rule 2 at a record literal's field, and they compile.
    ///
    /// The refusal and its menu are the kernel's since RFC-0125 §3 M3, rows 01,
    /// 02, 03, 27 and 34; `vyrn-cli/tests/refusals.rs` asks them of the whole
    /// compiler.
    #[test]
    fn a_stored_borrows_two_fixes_both_work() {
    ...
    fn a_loop_over_a_temporary_owns_its_elements() {
        ...
        // still owns, so a loop over one still borrows, and storing the element
        // is refused. The sentence is the kernel's since RFC-0125 §3 M3, rows
        // 01, 02, 03, 27 and 34, so `tests/refusals.rs` asks it of the whole
        // compiler.
    }
    ...
    // first where the root's owner is this frame, and never where it is not.
    // Both halves are the kernel's since RFC-0125 §3 M3, rows 01, 02, 03, 27
    // and 34, and `tests/refusals.rs` asks them of the whole compiler.
    ...
    fn a_map_key_may_not_be_a_borrow() {
        ...
        // spellings of the borrow — the element read inline, and the `for`
        // variable over a borrowed container — are the kernel's refusal since
        // RFC-0125 §3 M3, rows 01, 02, 03, 27 and 34, and `tests/refusals.rs`
        // asks both of the whole compiler.
        //
        // A copy is a value of this frame's, and so is a key with no other
        // owner — neither is refused, and that is what is left to ask here.
