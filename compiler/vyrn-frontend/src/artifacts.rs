...
impl ArtifactMap {
    ...
    pub fn display_path(&self, path: &str) -> String {
        ...
        // `" at "`) re-display as the readable form. Nested generation nests
        // banners — the importer of an inner banner is the outer banner — so
        // every INNER separator renders as ` at ` too (as loader's own banner
        // display does); only the last one separates head from importer and
        // is stripped, the rest are replaced, and no invisible U+001F
        // survives in a printed diagnostic.
        if let Some(at) = crate::loader::generated_importer(&path) {
            ...
            let head = head
                ...
                .unwrap_or(head)
                .replace(crate::loader::GEN_SEP, " at ");
...
mod tests {
    ...
    /// A deeper file sharing the entry key's TAIL is not that entry point —
    /// the lookup compares whole paths against the project directory, never
    /// a component-boundary suffix (which any `<dir>/main.vyrn` satisfied
    /// against a bare `main.vyrn` entry).
    #[test]
    ...
    /// Nested generation nests banners, and every layer's separator must
    /// re-display as the readable ` at ` — an inner U+001F surviving in the
    /// printed chain is invisible mid-text in diagnostics.
    #[test]
