...
// The generation machinery, declared in `gen.rs` since RFC-0125 §3 M5's
// thirteenth slice. None of it walks a tree; this file reads it the way it read
// it when the two lived in one file.
use crate::gen::{
    gen_code_splice, gen_module_interface_lit, gen_scoped_path, no_splice_rule, render_code,
    CodePiece, GenInputs, GenOutput, GenRead, Spliced,
};
...
/// The RFC-0054 splice rule for a tree-walker value.
///
/// THE one conversion (RFC-0125 §3 M5, the thirteenth slice). The rule itself
/// reads six cases, which are the six `vyrn_codegen::TAG_*` enumerates, so the
/// compiled generation engine builds a [`Spliced`] straight from its tag and
/// names no interpreter type at all. A `Val` with no case is refused HERE,
/// because only this side knows what an Array is — with `gen::no_splice_rule`
/// writing the sentence, so there is one wording and not two.
fn code_splice(val: &Val, ctx: i64) -> Result<Vec<CodePiece>, Ctrl> {
    let one = match val {
        Val::Str(s) => Spliced::Str(String::clone(s)),
        Val::Code(pieces) => Spliced::Code(pieces.clone()),
        Val::Bool(b) => Spliced::Bool(*b),
        Val::Int(n) => Spliced::Int {
            v: *n,
            signed: true,
        },
        Val::IntN { v, signed, .. } => Spliced::Int {
            v: *v,
            signed: *signed,
        },
        Val::Float(f) => Spliced::F64(*f),
        Val::Float32(f) => Spliced::F32(*f),
        other => return Err(no_splice_rule(val_kind(other), ctx).into()),
    };
    gen_code_splice(&one, ctx).map_err(Ctrl::Err)
}
