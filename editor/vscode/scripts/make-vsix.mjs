// Package the Vela VS Code extension + a bundled `vela-lsp` server into a
...
// `vela-<version>-<target>.vsix` in editor/vscode/.
//
// `extension.js` resolves the server as: the `vela.serverPath` setting, else
// the bundled `./server/<exe>` (this step), else the dev build at
// <repo>/compiler/vela-lsp/target/debug/<exe>.
...
const repoRoot = path.resolve(extDir, "..", ".."); // the Vela repo root
const lspManifest = path.join(repoRoot, "compiler", "vela-lsp", "Cargo.toml");

const isWin = process.platform === "win32";
const exe = isWin ? "vela-lsp.exe" : "vela-lsp";
...
// it there at runtime).
const built = path.join(repoRoot, "compiler", "vela-lsp", "target", "release", exe);
...
console.log(`\nDone. Install with:  code --install-extension vela-${target}-${readVersion(extDir)}.vsix`);
``