...
*[Amended: fixed after M4.] `vyrn why --capability` now walks the LINKED graph.
`loader::capability_graph` returns the load's own `floor::Graph` — the triples
`floor::objection` refuses over, one function building both — so the module set
is shared and the gate holds for the graph as well as for the vocabulary. The
load runs with the fence and the floor DISARMED, because both are refusals over
that graph rather than facts about it and a report has to be able to answer for
the tree that was just refused; a project that cannot LINK is now an exit 2 with
the loader's own diagnostic, which is the cost of the change and is narrower
than it sounds — the checker never ran here either way. Chains render through
`ArtifactMap::display_path`, the floor's own spelling, and a generated module's
banner keeps naming the author's call site. `shelf` answers:*

```
$ vyrn why --capability extern client
N:/lang/examples/shelf/client/boot.vyrn
  artifact: `client` (browser) — target `browser` has `extern`
  `vyrnRpcCall` needs `extern` — generated by client("../server/api") at client/boot.vyrn:37
