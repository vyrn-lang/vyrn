...
| a record literal's field | "`x` may not be stored into the field `R.s` — it is a `read` parameter" | "… into **the literal** — …", the same menu |
...
**The one wording that moves is the record literal's field.** `Rhs::Make` is "a
literal" to the kernel, so a store into one is "the literal" where the checker
