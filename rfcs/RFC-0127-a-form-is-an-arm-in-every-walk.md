...
941 mentions in eight files for 38 forms. The table is generated by
...
|---|---|---|---|---|---|---|---|---|---|
| `Stmt::Let` | 8 | 5 | 2 | 0 | 7 | 2 | 7 | 1 | 32 |
| `Stmt::Assign` | 3 | 3 | 2 | 0 | 5 | 1 | 5 | 0 | 19 |
| `Stmt::SetField` | 5 | 3 | 2 | 0 | 4 | 1 | 3 | 0 | 18 |
| `Stmt::IndexSet` | 4 | 3 | 2 | 0 | 4 | 1 | 2 | 0 | 16 |
...
| `Expr::Var` | 26 | 22 | 13 | 0 | 25 | 4 | 32 | 0 | 122 |
...
census ran from 7 mentions to 244, a factor of 35. This one runs from 13 to 122,
and once `Expr::Var` and `Expr::Call` are set aside — every walk names those two
for reasons of its own — from 13 to 34, with 24 of the remaining 36 rows under
24. A type constructor's cost tells you something about the constructor. A
form's cost mostly does not.

### 3.1.1 The floor, and what is above it

`Stmt::Continue` costs 14 mentions. A `continue` has no operand, no type, no
value, no ownership effect and no diagnostic of its own; the checker asks
whether a loop encloses it and every other pass steps over it. So **14 is the
price of existing**, and the rows above it pay 14 plus what they decide:

| form | all eight | above the floor | what the excess buys |
|---|---|---|---|
| `Stmt::Continue` | 14 | 0 | the floor |
| `Stmt::Break` | 15 | 1 | one more arm, in `own` |
| `Stmt::Drop` | 15 | 1 | RFC-0114's explicit reclaim and the capture rule, net of the arms `typed`'s loop scan gives a `continue` and not a `drop` |
| `Stmt::Let` | 32 | 18 | a binder, a type, a placement, a copy rule |
| `Stmt::Return` | 23 | 9 | the release of every live binding on the way out |

`Stmt::Region` sits on the floor too, at 14. An expression's floor is 13, held
by `Expr::Try` and `Expr::TryConstruct`, and it is now BELOW the statement floor
where it used to be above it: what made an expression the dearer of the two was
the collectors, and RFC-0125 §3 M6 folded them onto one descent. `Pattern::Other`
is 13 as well, so the three floors have met.

**Where the floor comes from.** It is not the deciders. Taking `Stmt::Region`'s
14 apart: two mentions are the parser building it and the checker's own rule;
the rest are exhaustive walks that do nothing with a region except recurse into
its block. It was 33 over six such walks: `collect_regex_block`,
...
| `Stmt::Break` | `break` | RFC-0060 | stays | 15 mentions, 14 of them §3.1.1's floor; nothing else leaves a loop early |
