...
`Stmt::Continue` costs 22 mentions. A `continue` has no operand, no type, no
value, no ownership effect and no diagnostic of its own; the checker asks
whether a loop encloses it and every other pass steps over it. So **22 is the
price of existing**, and the rows above it pay 22 plus what they decide:

| form | all eight | above the floor | what the excess buys |
|---|---|---|---|
| `Stmt::Continue` | 22 | 0 | the floor |
| `Stmt::Break` | 23 | 1 | one more arm, in `own` |
| `Stmt::Drop` | 23 | 1 | RFC-0114's explicit reclaim |
| `Stmt::Let` | 40 | 18 | a binder, a type, a placement, a copy rule |
| `Stmt::Return` | 35 | 13 | the release of every live binding on the way out |
