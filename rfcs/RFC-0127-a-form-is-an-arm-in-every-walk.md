...
census ran from 7 mentions to 244, a factor of 35. This one runs from 16 to 165,
and once `Expr::Var` and `Expr::Call` are set aside — every walk names those two
for reasons of its own — from 16 to 58, with 33 of the 38 rows between 28 and
58. A type constructor's cost tells you something about the constructor. A
form's cost mostly does not.

### 3.1.1 The floor, and what is above it

`Stmt::Continue` costs 28 mentions. A `continue` has no operand, no type, no
value, no ownership effect and no diagnostic of its own; the checker asks
whether a loop encloses it and every other pass steps over it. So **28 is the
price of existing**, and the rows above it pay 28 plus what they decide:

| form | all eight | above the floor | what the excess buys |
|---|---|---|---|
| `Stmt::Continue` | 28 | 0 | the floor |
| `Stmt::Break` | 29 | 1 | one more arm, in `own` |
| `Stmt::Drop` | 29 | 1 | RFC-0114's explicit reclaim |
| `Stmt::Let` | 45 | 17 | a binder, a type, a placement, a copy rule |
| `Stmt::Return` | 41 | 13 | the release of every live binding on the way out |

An expression's floor is 32 (`Expr::TryConstruct`), and it is higher for the
...
lowest rows — `Pattern::Other` at 16, `Pattern::Failure` at 17,
`Pattern::Success` at 22 — are the only rows under the statement floor.

**Where the floor comes from.** It is not the deciders. Taking `Stmt::Region`'s
33 apart: two mentions are the parser building and the checker's own rule; the
