...
`compiler/vyrn-cli/tests/pins/declarations.tsv` counts them in seven files. The
rows are `Program`'s `Vec` fields, read out of `ast.rs` by the test, so a tenth
declaration form fails the census until it has a row.
...
`compiler/vyrn-cli/tests/pins/keywords.tsv` counts the keywords in three files.
The spellings and the tokens are read out of the lexer's `keywords!` table,
which is the same anchor `editor/vscode/test/grammar.test.mjs` reads; this
census is that table's third reader.
...
`compiler/vyrn-cli/tests/pins/contextual.tsv` counts the contextual words in
four files: the words the lexer hands back as identifiers and the parser reads
by position.
