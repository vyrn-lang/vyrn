...
| [0117](RFC-0117-a-key-is-anything-that-hashes.md) | a key is anything that hashes | Implemented; `Hashable` + `Map<Int64, V>` in all four engines, the wire form built (stringified keys, canonical decimal), and M2's user keys landed — heapless records and fieldless enums via canonical packs, payload enums waiting by name |
...
| [0125](RFC-0125-a-rule-is-stated-once.md) | a rule is stated once | Draft; a named, place-based core, a kernel of three judgments, one emitter and a runtime in Vyrn — M0 to M7 landed: every body lowers from the core's rows and the AST walk is deleted (#545); the probe that motivates it is in §1.4 |
