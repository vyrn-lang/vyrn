...
**The line is fixed; the work waits for a caller.** Vyrn will only ever do DECIDABLE subsumption — option 1 (numeric range containment) and option 2 (DFA containment over string predicates), the two the file shows need no solver and no backend — and will never take option 4: an SMT solver is a dependency, a nondeterminism, and a trust root this compiler's word-is-checkable philosophy cannot absorb. Option 5 (user-declared subsumption) falls with it, because verified declarations reduce to options 1–3 anyway and unverified ones are option 4 without the solver. Nothing is implemented today: `assignable`'s name-identity rule has not yet refused a program anyone wrote, and a subsumption pass with no caller is speculation. The direction is recorded so that when a program does hit the wall, the answer is already chosen and bounded.
