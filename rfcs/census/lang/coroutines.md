...
**Deferred — `spawn` stays the concurrency story.** Every option fails a constraint this repository holds harder than convenience. Stackful (C) and the proposal-gated variant (D) break the three-backend byte-identity that parity pins: wasm has no stack switch today, and betting the language on a Phase 3 proposal is the census's own counter-argument. Stackless (B) is a new compiler pass, function colouring, and an answer for borrows across suspension that would reopen RFC-0114's model — the largest possible change for a need no program has demonstrated: nothing in the tree wants concurrent I/O that fork-join `spawn` does not give. Reusing `gen` (E) mistakes shared spelling for shared machinery, as the file itself concludes. Reopen when BOTH are true: a real program needs suspension, and either wasm stack-switching ships in wasmtime by default or a stackless design exists that survives the move checker without an effect system.
