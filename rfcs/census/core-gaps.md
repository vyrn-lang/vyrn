...
The third small-clause track (`m7-tail2`) changed no gap. It took three clauses of the fourth count. The element clause was already paid on main: `m7-read`'s owned clause never asked the element's type, so `diffEl` and `diffPositional` were taken, and `coredrive` gains the enum and array shape as a witness. The screen asks about a `modify` of an alias's root only over the rows of the alias's extent, through `kernel::modifies`, the kernel's `modify` write points; `vondemo`'s `rawNumber` is taken. A call to a generic function carries the type arguments the checker solved at the call (`Row::solved`), and the emitter calls that instance and solves nothing; seven bodies are taken, among them the `Slots` callers. What stays at the call is a call through a function value, 8 bodies, and `isSuccess` at a `?` on a generic `Fallible`, 2 bodies. The screen refuses 964 bodies where it refused 972 on main `784cb9f5`, 769 with no gap where 777. `coredrive --ignored` reads 21,333 with no gap and 1,643 carried end to end, both unmoved, and the emitter takes 20,451 bodies where it took 20,444.

The route track (`m7-route`) retired `Call:Builtin:lineAt`, `Call:Builtin:colAt`, `Call:Builtin:toJson` and `Call:Builtin:fromJson`. `lineAt` and `colAt` are `Spec::Routes` rows to `std/text`'s `lineAtV` and `colAtV`, and the runtime's own pair went. `loader::routed_callee` names the function `contractOf`, `toJson` and `fromJson` call, and the builder states `Callee::Fn` where the program declares it. Over the 415 roots the lines with a gap fall from 844 to 617: the line and column group from 51 to 0 and the JSON group from 178 first to 0. `contractOf` stays at 80: every one is a host program's copy of a gen body, which declares no entry, while the generator program's copies are whole. `coredrive --ignored` reads 21,034 bodies whole of 21,220 and 1,669 carried end to end, where main read 21,340 of 21,556 and 1,648, and the emitter takes 20,179 bodies of 21,176 where main `bfe93bd4` took 20,487 of 21,512; the 336 bodies fewer are the runtime pair.

The small-readers track (`m7-small`) retired `Make:Try` and `Call:Builtin:serveStream`, and all but a few lines of `Call:Builtin:@at`, `Call:Builtin:jsonSchema` and `Call:Method:*`. A heapless element read off a receiver that is no place binds the receiver to a temporary and releases it after the read, as a field read does; `serveStream` is a `Spec::Traps` row whose operand is the frontend's sentence; `jsonSchema<T>()` is the String literal its declaration renders; `T?(v)` is a made layout whose reader calls the arm's one check; and a method is `Callee::Fn` to the one non-generic impl function its receiver's type key names. Over the 415 roots with the generator cache off, on main at `a5d4fa5f`, the lines with a gap fall from 980 to 857: `@at` 81 to 23, a user container's `place at` or a projection; `serveStream` 20 to 0; `jsonSchema` 17 to 0, with `schemaOf`'s 7 left for its memo; `Make:Try` 11 to 0; a method 38 to 4, each a generic impl. `coredrive --ignored` reads 21,366 bodies whole where it read 21,340 and 1,659 carried end to end where it read 1,648, and the emitter takes 20,456 where it took 20,444; rebased onto `7cbd2e29`, `coredrive --ignored` reads 20,988 whole where it read 20,962, 1,637 carried end to end where it read 1,626, and the emitter takes 20,193 where it took 20,179.

The retirement track (`m7-retire`) changed no gap and deleted no arm. vyrn-codegen's unit tests compiled with no core and reached every arm of the AST walk, 182 of `Stmt::Break`'s 187 occurrences among them; since this track they install the core, as the CLI does. Over the whole gate list `VYRN_FORM_TALLY` still reads every arm above zero except `Stmt::Continue`, which `track-ek` retired. The arms nearest zero are `Stmt::Break` at 5, all `jchain.vyrn`'s `main`, where the core states no run at the `break`; the `try` constructor at 55; `Stmt::Region` at 56; `Expr::Float` at 321; a lambda at 349; and `Stmt::IfLet` at 1,047. `Stmt::SetField`, `Stmt::IndexSet`, `Stmt::Region` and `Stmt::Drop` have no pairing in the per-statement screen, so each leaves its arm only when the whole-body walk takes its body. The record's tables name the clause and the bodies for each arm.
