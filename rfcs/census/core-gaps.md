...
The alias track (`m7-alias`) changed no gap: every body it moved was whole in the rows already and stood down at the emitter's screen. A borrow bound by a read of a layout holds the place's address, a length is a header read, a String operator reads the rows, and a layout crosses a `modify` or `consume` parameter as its address. Over `examples/` a read of a layout was the first name the screen refused in 1,365 bodies and is in 30. `coredrive --ignored` reads 16,563 with no gap and 1,044 carried end to end, both unmoved, and the emitter takes 15,120 bodies where it took 14,798. The largest first refusals left are a builtin's call result, 3,583, and the scalar element read, 1,165 statements, which the rows cannot take until they state the header hoist the arm makes for a loop that never moves its array.
