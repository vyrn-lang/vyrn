...
The third screen track (`m7-screen3`) retired `Switch:Impl`. A `?` on a declared `Fallible` states the impl's `isSuccess` call and its negation before the switch, and the failing arm tests that name (`Test::Holds`); the two impl calls are `Callee::Fn`, functions the program declares, where `Callee::Method` read as a callee gap. The failing arm of a `?` on an `Option` returns `None` of the frame's result, and the emitter's screen reads a switch on a payload binder as a switch on a place. `core::cut` ends a list at an `if` or a `switch` whose every arm traps, which `substring`'s nested `match` needed once the screen admitted it. `coredrive --ignored` reads 21,333 bodies whole where it read 21,330 and 1,643 carried end to end where it read 1,642, and the emitter takes 20,060 of 21,512 where it took 20,034 on `m7-hole`. The section above tables what the screen refused before the track moved: made layouts lead, then `@str` of a String temporary and the failing `return` of a `?` on a `Result`.
