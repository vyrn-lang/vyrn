...
- **Status:** **Implemented** (M0–M3) — see [M0 — as landed](#m0--as-landed),
  [M1 — as landed](#m1--as-landed), [M2 — as landed](#m2--as-landed),
  [M3 — as landed](#m3--as-landed) and `rfcs/probe-0107/`. `std/vyx` names no
  component, `std/icons` is a library any `.vyrn` file can import, `<Icon>` is a
  provider a PROJECT writes in three lines, and the site draws six glyphs through
  it — counted, so the artifact carries those and no others. A milestone that
  fails its gate says so in this file.
...
The `<Icon>` provider, and the site as its first consumer. Four things: the
provider layer in `std/icons`, a project provider module of three lines, six real
tags in the site's own templates, and a one-word change in `std/vyx` that is a bug
fix and not a preference.

### The provider does not read the collection — it emits an import

M2 closed on a contradiction it could not resolve: a tag's attributes reach a
provider as ONE JSON string, an alias inside that string is not a constant path
argument, so it declares no input root and M2's import-map step never sees it. M2
offered "a provider module per collection" as the way out. It was not needed, and
the reason is M0's verdict (b′) applied one level deeper.

**A provider reads nothing.** What `iconProvider` returns is a MODULE that
imports the core generator, with the collection as that nested generation's own
constant argument:

```
// ==== generated by Icon("{\"name\":\"bytesize:github\"}", "./app/routes/layout.vyx", 131, 1) at … ====
/// `<Icon name="bytesize:github"/>` at ./app/routes/layout.vyx:131 — generated by std/icons.
import { iconsAt, iconAttrs } from "std/icons"
import { Html } from "std/html"
import * as g from iconsAt("bytesize", "github", "./app/routes/layout.vyx", 131, 1)
export fn provide() -> Html {
    return iconAttrs(g.github(), "", "", "")
}
```

`"bytesize"` is a `vyrn.json` dependency alias, and it is spelled in the one
position where M2's machinery already works: as `iconsAt`'s own constant
argument. So the import map resolves it, the resolved key goes into the input
roots AND into that generation's cache key, and the input-root rule still decides
on the resolved key. Nothing was needed in the compiler, in `std/vyx`, or in the
sandbox. The chain is three generations deep — `components`/`pages` → `Icon` →
`iconsAt` — and every link is the same mechanism: a generator that cannot open a
module its caller chose emits an import of it instead.

The collection therefore never has to ride the attribute JSON, and M2's
contradiction 1 is answered by not doing the thing it said could not be done.

### The project's provider module, and why the vocabulary lives there

The prefix vocabulary is the one thing a library cannot supply: `bytesize` and
`codex` are keys in the PROJECT's `vyrn.json`. So the provider is a project
module, and `std/icons` makes it three lines (`site/app/icons.vyrn`, its doc
header not shown):
...
The tag's name is the exported name — `Icon` because that file says `Icon`, and a
project wanting `<Glyph>` renames the function. The collections are that
generator's own constant argument, in the project's own file, which is where a
name only the project knows belongs. `std/icons`'s header carries this recipe in
full, because a third party has to be able to reproduce it from the doc alone.

Three exports carry it, and none of them knows what `.vyx` is:

- **`iconProvider(attrs, file, line, col, collections)`** — reads the attribute
  object with `std/jsonread`, applies the prefix rule, emits the module above.
- **`iconsAt(collection, names, anchorFile, line, col)`** — M2's `icons` with an
  anchor, so a report about a glyph lands on the tag rather than in the
  collection file.
- **`iconAttrs(glyph, size, label, class)`** — the only part of `std/icons` that
  runs outside a generator: it rewrites the drawn `<svg>`'s attributes with the
  use site's.

### The entry point is now `provide`, and the rename is a bug fix

M1's protocol said the generated module exports `render() -> Html`. Every green
row M1 wrote had TWO provider tags. With exactly ONE, the build fails — inside
`std/vyx`, about a function nobody called:

```
std/vyx.vyrn:1798:0: function `render` is defined in `generated by Glyph("{\"name\":\"github\"}", "./comp/Badge.vyx", 6, 1) at generated by components("./comp") at app.vyrn` but not imported here — add it to an `import { .. } from` list
```

**This is not about providers.** `render` is one of RFC-0054's four SURFACE
BUILTINS (`render`, `rawAt`, `raw`, `lex`), which are deliberately not reserved
because they are common words: a same-named user function is MEANT to win. But
the shadowing is decided over the whole PROGRAM rather than per module, so a
function named `render` anywhere in the graph takes the name away from
`render(vyrn"…")` in every OTHER module. With no providers involved at all, this
is the whole of it:

```vyrn
// poison.vyrn
export fn render() -> Int64 {
    return 0
}
```

Import that from a program that also uses `std/vyx`, and `std/vyx` stops
compiling with the message above. It reproduces on `main`; it has nothing to do
with this RFC; a generated provider module exporting `render` was merely the
first thing in the repository to walk into it. Two tags dodged it by accident —
two modules define the name, the loader's clash path grants visibility, and the
builtin survives.

M3 renamed the conventional entry point to `provide`: one word in `std/vyx`'s
emitter, the protocol doc beside it, and M1's tests. The row M1 was missing is
now there (`one_provider_tag_alone_in_a_page_builds`).

The compiler fault is RECORDED, NOT FIXED. Making the surface builtins
module-scoped means the checker knowing which module each function belongs to,
which is a change to name resolution and belongs to RFC-0054. A one-line loader
guard was tried and rejected: it silences the loader's half and leaves the
checker resolving `render(vyrn"…")` to the user's zero-argument function, so the
same program then fails with "`render` expects 0 argument(s), got 1" from the
same line. Half a fix is a worse message.

### The tag, and the two a11y rows

```
<Icon name="bytesize:github"/>
<Icon name="bytesize:external" label="opens on github.com"/>
```

`name` is required and carries the collection prefix; `size` (any CSS length),
`label` and `class` are optional; every other attribute is refused BY NAME, at
the tag. What is drawn is `std/icons`'s own markup — a `viewBox` from the
collection, `1em` in both dimensions, no `fill` — so a glyph takes the colour and
the size of the text around it, and the palette tokens and the theme control
reach it with no per-icon work.

The `label` is the whole accessibility rule, and both halves are on the site:

```html
<svg viewBox="0 0 64 64" width="1em" height="1em" aria-hidden="true">…</svg>
<svg viewBox="0 0 32 32" width="1em" height="1em" role="img" aria-label="opens on github.com">…</svg>
```

An icon BESIDE a text label is decoration and is hidden, because a reader who
hears "GitHub GitHub" has been told nothing twice. An icon carrying meaning that
no word beside it carries — the arrow saying this link leaves the site — is
content, and is named. A `label` moves a glyph from the first row to the second:
`role="img"`, that `aria-label`, and `aria-hidden` DROPPED rather than left to
contradict it.

### The site, as the first consumer

Two collections, pinned with `vyrn add` at the repository root — which is the
manifest every path here resolves against, the site having none of its own:

```
vyrn add github:iconify/icon-sets@66114542c442d138c1da78932ddbad862fb7a65c/json/bytesize.json --name bytesize
vyrn add github:iconify/icon-sets@66114542c442d138c1da78932ddbad862fb7a65c/json/codex.json  --name codex
```

Both are hash-locked in `vyrn.lock` (`33c635f7…` and `09a30b8c…` — the same bytes
M2's gate ran on) and read from the content cache; the export fetches nothing.
They are NOT vendored into the repository: `vyrn vendor` fetches everything the
lock pins, and this lock pins the toolchain too, so vendoring would put 150 MB of
archives in git. The two 22 kB collections are fetched once per machine instead,
hash-verified against the lock. `vyrn_vendor/` is now in `.gitignore`, which it
should have been before this.

Six tags, in two templates:

| where | tag | kind |
| --- | --- | --- |
| the masthead's search opener | `bytesize:search` | decorative |
| the footer's source link | `bytesize:github` | decorative |
| the same link, after the words | `bytesize:external`, `label=` | LABELLED |
| the footer's backstage link | `codex:checklist` | decorative |
| `/install` — "The one line" | `bytesize:download` | decorative |
| `/install` — "It refuses what it cannot check" | `bytesize:lock` | decorative |

Four are in the shell, so they are on all 67 consumer pages; two are on one
route, which is what proves a ROUTE's own `<script>` reaches a provider exactly as
the layout's does.

The OS tiles the milestone line named are NOT converted, and could not be: the
tiles are a `v-for` over `platforms()`, so a per-tile glyph is `:name` bound to an
expression, which M1 refuses structurally. A per-platform glyph needs three static
tags or a sibling `.vyx` component, and the tiles are better left alone than
restructured to hold an icon.

### The diagnostics, verbatim

A misspelled glyph travels the whole chain — the provider handed the anchor to
`iconsAt`, which handed it to the emitter, which reported there. This one is from
the site itself, with `bytesize:github` mistyped in `layout.vyx`:

```
site/app/routes/layout.vyx:131:1: the collection `bytesize` has no glyph `githup` — nearest is `github`
  note: in generated code generated by iconsAt("bytesize", "githup", "./app/routes/layout.vyx", 131, 1) at generated by Icon("{\"name\":\"bytesize:githup\"}", "./app/routes/layout.vyx", 131, 1) at generated by vyxLayout("./app/routes/layout.vyx") at generated by pages("./app/routes") at site/export.vyrn:4 (see `vyrn emit-gen`)
```

The provider's own five, each at the tag, from a fixture binding the same two
collections:

```
Badge.vyx:6:1: `<Icon>` has no `colour` attribute — it takes `name`, and optionally `size` (a CSS length), `label` (the accessible name) and `class`
Badge.vyx:6:1: `brand:github` names the collection `brand`, and this provider binds `bytesize`, `codex` — write one of those, or bind `brand` in the provider module
Badge.vyx:6:1: `github` names no collection, and this provider binds `bytesize`, `codex` — write `bytesize:github` for one of them
Badge.vyx:6:1: `<Icon>` needs a `name` — the glyph to draw, `name="bytesize:github"`
Badge.vyx:6:1: `bytesize:` names a collection and no glyph — write `bytesize:github`
```

Two of them name the vocabulary, because the vocabulary is the project's and no
reader can guess it. A bare name is refused rather than resolved by position when
more than one collection is bound; with exactly one bound, `name="github"` means
that one.

And M1's structural refusals are what a provider tag gets for free — the first of
them is why the OS tiles stayed as they are:

```
Badge.vyx:6:1: `<Icon>` is a generation-time provider, and `:name` binds an expression — a provider's attributes become constant arguments to a generator, so write `name="…"` as a static attribute, or wrap `<Icon>` in a sibling `.vyx` component that computes it
Badge.vyx:6:1: `<Icon>` is given children, and it is a generation-time provider — a provider's tree comes from its attributes alone, so it takes none
```

### The counted gate, as run

`site/test/icons.test.mjs`, over the exported tree, taking every number from the
TEMPLATES so that adding a tag moves the expectation with it:

- **Every page carries exactly the glyphs its templates name.** All **175**
  exported pages: a page wearing the consumer shell expects the shell's tags,
  `/install` expects two more, a backstage page (its own masthead, no shell)
  expects none. **270 glyphs = 67 × 4 + 2**, and every page is checked
  individually rather than in the total.
- **The export carries no glyph the templates did not name.** Distinct glyph
  BODIES across the whole tree: **6**, against **6** distinct names in the
  templates. The two collections hold **180** glyphs between them, so 174 of them
  are not in the artifact — the "only named glyphs are generated" claim, measured
  at the far end.
- **A decorative glyph is hidden and a labelled one is named.** Every drawn glyph
  is `aria-hidden` XOR `role="img"` with a non-empty `aria-label`; the labelled
  tags and the named glyphs agree one for one, BY THEIR LABEL TEXT; and no glyph
  carries a `fill` or a pixel size, which is what makes the theme control reach
  them.
- **A template that gains tags cannot skip this gate.** The first row asserts
  that the set of templates carrying `<Icon>` is exactly the set this file maps
  to pages, and names the file that drifted.

### What it costs

One generation per tag, and each generation reads and parses the whole collection
— M2's quadratic `parseJson` wall, unmoved. Measured on two tags over the two
collections, cold cache: **14.7 s**; the same build warm: **0.080 s**. The site's
six tags cost about forty seconds on a cold generator cache and nothing after,
which is what a CI run pays once. M2 measured the same reads at 0.4 s under the
RFC-0076 compiled engine (`--features wasm-gen`); which of those two facts to
change is still RFC-0082's and RFC-0076's question, not this one's.

### Gate results

- **The site export:** 175 routes, 13 assets, green, and offline — the two
  collections resolved from `vyrn.lock` and the content cache with no fetch.
  `site/test/*.test.mjs`: **31 passed, 0 failed** (4 of them the new counted
  gate; the contrast checker is among the other 27, and `currentColor` made it
  free). The site's own `vyrn test` loop: 26 files, **188 blocks ran, 0
  failed**. The guide's programs: green.
  `vyrn fmt --check site/app/*.vyrn site/guide/*.vyrn site/export.vyrn`: clean.
- **`std/icons`'s own `test` blocks: 22 passed, 0 failed** — M2's 14 and 8 for
  the provider layer. The provider reads nothing, so its whole surface is a
  function of the attribute JSON and the rows need no file, manifest or cache.
- **`std/vyx`: `cargo test -p vyrn-cli --test vyx` 45 passed, 0 failed** — M1's
  44 and the one-tag row.
- **Existing generators byte-identical.** `vyrn emit-gen` over all **221**
  examples, this branch's `std/` against `main`'s, one and the same compiler
  binary, each side with its own `VYRN_GEN_CACHE_DIR`: **0 differing**. No
  example names a provider tag, so nothing should have moved, and nothing did.
- **Workspace `cargo test --release`:** 72 suites, **1,760 tests, 0 failed**.
  **Parity** (`--ignored --test-threads=1`, tools pinned): **40 passed, 0
  failed**, 249.1 s.
- **Excluded crates:** `vyrn-lsp` 76 + 7 passed, 0 failed; `vyrn-genwasm` green; `vyrn-play`
  builds for `wasm32-unknown-unknown`. `cargo fmt --all --check` clean for the
  workspace and for the three excluded manifests.
  `vyrn doc --std -o docs/api --verify` up to date (39 files), committed.

### What M3 contradicts, and what it leaves

1. **M2's contradiction 1 is answered by not needing an answer.** M2 said M3 must
   either give the provider the collection as an argument of its own or grow the
   root rule. Neither happened: the provider takes no collection and reads
   nothing, and the collection is the argument of the generation it emits.
   `iconsModule` — the text-in surface M2 shaped for exactly this — is no longer
   on the path. It stays exported for a caller that HAS the bytes and no
   manifest, and its rows are the emitter's tests.
2. **M2's contradiction 3 is closed where M2 said it belonged.** The prefix
   vocabulary is not in the core: `icons` still refuses a `:` in a glyph name,
   and the splitting lives in the provider layer, which is what parses the tag.
3. **M2's contradiction 2 is unchanged.** A report about an alias-named
   collection that cannot be READ still anchors at the alias spelling, which is
   not a file an editor can open. Every report on the working path anchors at the
   tag, so this is reachable only when the collection itself is missing.
4. **The `render` shadowing fault is filed, not fixed.** It is the second
   compiler fault this RFC has walked into and left alone; M0's contradiction 4
   (a checker error inside a generator module attributed to the importing file)
   is the first, and both are still true.
5. **M1's `col` is still 1**, so every diagnostic above reports column 1. The
   protocol still carries the argument, and needs no change when a template node
   learns a column.
6. **The milestone line named the OS tiles and they are not converted** — the
   reason is above, and it is the static-attribute rule doing exactly what it was
   built to do.
...
**M3 — the `<Icon>` provider and the first consumer.** **Landed** — see
[M3 — as landed](#m3--as-landed). The provider layer is `std/icons`'s, the
provider MODULE is the project's and is three lines, and the collection reaches
it as the constant argument of the generation it emits rather than through the
attribute JSON — which is how M2's contradiction 1 was answered without giving
the root rule anything new. Gate met: the counted gate is
`site/test/icons.test.mjs`, taking its numbers from the templates (270 glyphs on
175 pages, 6 distinct bodies against 6 distinct names, out of 180 glyphs in the
two collections), and both a11y rows are on the site and asserted. Two
corrections: the OS tiles are NOT converted, because they are a `v-for` and a
provider tag takes static attributes only; and the protocol's entry point is now
`provide` rather than `render`, because `render` is one of RFC-0054's surface
builtins and the shadowing that gives a user function the name is decided over
the whole program — so a page with exactly ONE provider tag did not build, which
every M1 row missed by having two. The original line was:
The provider over the
