...
> client (wasm). The client half exists: a Vela module runs in a page, calls
> JS, and is called back. This RFC ships the server half's v1 — and settles
> the question it forces: does Vela need `async`/`await`? **No** (for now,
...
RFC-0013 decided the browser shape: the host owns the loop; Vela owns state
...
```
velac serve examples/server.vela --port 8080
```
...
- **`fn handle(req: Request) -> Response`** — an ordinary Vela function. No
  new declaration form, no attributes: `velac serve` requires the root file
...
## `velac serve [file] [--port N]`

A hand-rolled HTTP/1.1 host inside `velac` (Rust `std::net` only — the
...
- Parses the request line, headers, and a `Content-Length` body. Responds
  ...
  400 without reaching Vela; a *trap* inside `handle` (validation failure,
...
**Vela does not add `async`/`await` now.** Reasons, in order:
...
3. **Determinism is the product.** Vela's concurrency (`spawn`) is
...
runtime lands and wants an I/O-overlap story. Until then, "async Vela" is
spelled: start the work in the host, come back through a handler.

## Verification & parity

- `examples/server.vela` defines `handle` + a `main` that calls `handle`
  ...
  semantics).
- Integration tests in vela-cli: spawn `velac serve` on an ephemeral port,
...
- Live verification: serve the example and load it in the browser pane —
  Vela-rendered HTML over real HTTP.
