...
- **Status:** **Accepted**, M1 not started. Milestones are gated on measurement
  the way RFC-0081's were: nothing is deleted until a number says it should be.
...
census rows — in ordinary Vyrn over `Array<T>`. It runs today:
...
no capability, no new primitive.
