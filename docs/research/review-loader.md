...
Counts: 50 findings — **38 CONFIRMED**, **8 PLAUSIBLE**, 4 design critiques.
...
## Lens 5 — Agda implementer: soundness, proved with programs

Two claims were put under test. RFC-0072: the audience rule refuses every
widening edge. RFC-0033 and RFC-0053: an origin map sends a diagnostic to the
user source it came from, and never loses one. Both hold for the shape their
tests cover, and both break on a second shape.

### A5.1 CONFIRMED — Critical. A generator import edge is never checked, and a single-file argument lends the generated module its input's audience

Two facts combine.

1. `loader.rs:1160-1184` calls `audience_objection` for `ImportSource::Path`
   only. The generator loop at `loader.rs:1190-1213` checks nothing on the edge
   from the calling module to the generated module.
2. `audience.rs:284-302` (`source_file`) gives a generated module the audience of
   the generator's **input file** whenever the last component of the first string
   argument contains a dot; otherwise it inherits the caller.

So an input's audience is a privilege grant, and nobody checks the grantee
against it. The production shape is a `.vyx` page mounted per file. Manifest:
`{"client": "client/boot.vyrn", "audience": {"server": ["server"], "client":
["client"], "universal": ["app","shared"]}}`; `server/pages/Leak.vyx` has a
`<script>` importing `secret` from `../store` and a template that renders it.

```
$ cat client/boot.vyrn
import { page } from vyxPage("../server/pages/Leak.vyx")
import { vyxPage } from "std/vyx"
import { toHtmlString } from "std/html"
fn main() -> Int64 { print(toHtmlString(page())) return 0 }

$ cat server/store.vyrn
export fn secret() -> String { return "TOP-SECRET" }

$ vyrn check client/boot.vyrn
ok
exit=0

$ vyrn run client/boot.vyrn
<main><p>TOP-SECRET</p></main>
exit=0
```

The control isolates the cause exactly. The same program with a **directory**
argument, where `source_file` falls back to the caller, is refused:

```
$ vyrn check client/boot.vyrn          # import { leak } from mk("../server")
generated by mk("../server") at client/boot.vyrn:1:0: `client/boot.vyrn` is client-only and
cannot import `server/store.vyrn`, which is server-only
exit=1
```

`compiler/vyrn-cli/tests/audience.rs:252-296` proves this leak is caught — and it
mounts through `components("../app/widgets")`, a directory. Move the `.vyx` under
`server/` and mount it with `vyxPage` / `vyxPageClient` / `vyxLayout` (all
single-path generators, `std/vyx.vyrn:4154,4178,4267`) and the refusal
disappears. RFC-0072 M1's "as landed" note (lines 326-330) argues that the input,
not the call site, should decide a generated module's audience, and M5 (lines
518-537) argues the fallback for pathless generated modules. Neither records that
the caller-to-generated edge is unchecked, so this is a defect, not a decision.
The narrowest fix is one `audience_objection(key, &gen_key, ..)` call in the
generator loop; equivalently, take the narrower of caller and input.

### A5.2 CONFIRMED — High. The classifier decides by the key string, so a second spelling loses the audience

`audience.rs:339-350` (`classify` compares segments with `==`) and
`audience.rs:361-375` (`relative_to` uses `strip_prefix`). Nothing canonicalizes.
`normalize` (`loader.rs:168-191`) folds `.`, `..` and duplicate separators — it
does not fold case and does not resolve reparse points.

Case, on Windows, importing `"../../Server/store"` instead of `server/store`:

```
> vyrn check aud\p2\main.vyrn
ok
> vyrn run aud\p2\main.vyrn
RUN_EXIT=7                       # the server module ran
```

A directory junction (`mklink /J vendor server`), importing `"../vendor/store"`:

```
> vyrn check aud\p7\app\x.vyrn
ok
> vyrn run aud\p7\app\x.vyrn
RUN_EXIT=7
```

The tool contradicts itself, because `vyrn why` canonicalizes
(`main.rs:1285-1291`) and `check` does not:

```
> vyrn why aud\p7\vendor\store.vyrn
C:/.../aud/p7/server/store.vyrn
  audience: server-only — path segment `server` (vyrn.json audience.server)
```

On a case-insensitive filesystem this is a one-character edit. The rule is stated
on strings where it has to be stated on file identity.

### A5.3 CONFIRMED — High. Text inside a `.vyx` string literal becomes a compiler error at any path, with any wording

`origin.rs:283` scans **every** line of a synthesized module for `//@diag`, with
no lexical context, and `loader.rs:918-943` fails the load on `Severity::Error`.
`std/vyx` copies a `<script>` helper body verbatim (RFC-0048 §1), so a multi-line
string literal in a component file puts author-controlled text at the start of a
generated line:
...
fn banner() -> String {
    return "first
//@diag error ../../../../../../../../elsewhere.vyrn:1:1 injected by a .vyx string literal
last"
}
</script>
```
...
```
$ vyrn check app.vyrn
../../../../../../../../elsewhere.vyrn:1:1: injected by a .vyx string literal
  note: in generated code generated by components("./comp") at app.vyrn:10 (see `vyrn emit-gen`)
exit=1
```

No generator authored that report. The component did, through data it merely
passes through. RFC-0099:198 ("a generator is trusted like any other code the
project imports") does not cover it: the trusted party is `std/vyx`, and the text
came from a component file — the kind of artefact a project copies from a
gallery. The failed build names a file that is not in the project and a message
the compiler did not write.

### A5.4 CONFIRMED — High. An `//@origin` inside a generated string literal hijacks the map below it

The same root cause on the origin side. `origin.rs:96-101` line-scans the whole
text and `origin.rs:99` matches on `trim_start()` alone, so a directive inside
string data displaces the valid directive above it:

```
//@origin ./data/two_lines.txt:1:1
export fn s() -> String {
    return "a
//@origin ./no_such_file.vyx:7:7
b"
}
export fn f() -> Int64 { return nope() }
```

```
$ vyrn check app_instring.vyrn
no_such_file.vyx:7:7: call to unknown function `nope`
  note: in generated code generated by emit("instring") at app_instring.vyrn:7:1
```

The error is attributed to a file the generator never read. `std/vyx` happens to
be immune because it brackets every copied line with `//@origin end`. That
containment is one generator's emission style, not a property of the mechanism
RFC-0033 offers to any generator over any input format.

### A5.5 CONFIRMED — Medium-High. An over-deep `..` yields a relative origin key, which the LSP cannot publish anywhere

With an absolute importer — the shape the LSP always has — twenty `..` pop the
whole absolute prefix and nine survive (`origin.rs:399-405`), so `Origin.file` is
relative:

```
$ vyrn check C:\...\probe\app_escape_root.vyrn
../../../../../../../../../outside.vyx:3:5: call to unknown function `nope`
```

The CLI still prints it. The editor cannot: `symbols.rs:449-455` moves every
successfully remapped diagnostic into `Analysis.remapped` and **out of**
`Analysis.diagnostics`, and `vyrn-lsp/src/main.rs:3450-3464` publishes `remapped`
only `if let Ok(uri) = Url::from_file_path(file)`, which fails for any relative
path. The removal is unconditional; the publish is not. This is the "never lose a
location" guarantee failing, reached by reading rather than by driving the LSP,
so it is ranked below the CONFIRMED frontend behaviour.

### A5.6 CONFIRMED — Medium. The "the origin file does not exist" guardrail is not implemented

RFC-0033:85-87 states the rule: "if a directive is malformed **or the origin
file/position doesn't exist**, the diagnostic surfaces at the generated
location". `parse_origin_body` (`origin.rs:351-372`) validates syntax and
1-basedness only. Nothing stats the file or bounds the line:

| directive | reported at |
|---|---|
| `./no_such_file.vyx:5:3` | `no_such_file.vyx:5:3` |
| `./data/two_lines.txt:9000:400` (2-line file) | `data/two_lines.txt:9000:400` |
| `...:18446744073709551615:18446744073709551615` | the same, verbatim |

No panic — `99999999999999999999999` fails `usize::parse` and correctly falls
back with a `malformed` note, and `:0:0` is caught at `origin.rs:361`. But
`usize::MAX` parses, and in the LSP it reaches
`vyrn-lsp/src/main.rs:3571` (`d.line.saturating_sub(1) as u32`) and truncates to
4294967294 — a squiggle on a line that does not exist, invisible in the editor
and absent from `Analysis.diagnostics` (A5.5).

### A5.7 CONFIRMED — Medium. Anchors roam: `..` escapes the project and an absolute path passes through unjoined

RFC-0099:207-209 says "Anchors resolve, they do not roam … it cannot name a file
the user could not open". Two ways out: `origin.rs:399-405` pushes a `..` that
cannot be popped, and `origin.rs:378-382` returns the path verbatim when
`importer_dir` is empty — the common CLI shape.

```
$ vyrn check app_escape.vyrn
../../../../../../../../Windows/win.ini:2:4: call to unknown function `nope`
$ vyrn check app_absolute.vyrn
C:/Windows/win.ini:2:4: call to unknown function `nope`
$ vyrn check app_diag_escape.vyrn
../../../../../../../../Windows/win.ini:1:1: the generator chose this file and this wording
```

The last is a hard error, so the generator chose the severity, the wording, and a
file outside the project — and with A5.3 the author of that anchor need not be
the generator. Not checking that the generator READ the anchored file is a
recorded v1 decision (RFC-0099:136-140) and is a design critique. The claim that
an anchor cannot leave the project is a defect against the RFC's own text.

### A5.8 CONFIRMED — Medium. The audience vocabulary is never validated: a glob or a misspelled key turns the rule off

`from_manifest` (`audience.rs:148-193`) reads three fixed keys and keeps whatever
strings it finds; `classify` compares them literally. Nothing rejects an unknown
key, an entry that is not a path segment, or an empty list, and there is no
warning channel:

```
$ vyrn check app/x.vyrn         # "server": ["server/*"] — a plausible glob
ok
$ vyrn check app/x.vyrn         # "servers": ["server"] — one letter wrong
ok
```

In both cases `server/` is unprotected and the project looks configured. This is
the failure mode RFC-0072:105-109 criticises in Nuxt and Leptos ("a misconfigured
feature breaks it"), reached here through a typo. The opt-in design makes it
worse in one specific way: absence of the key is a legitimate, silent, supported
state (`audience.rs:142-148`), so "nothing was rejected" carries no information.
Pair with R3.2, where a trailing comma reaches the same state.

### A5.9 CONFIRMED — Medium. `vyrn why` loses every generator edge whose argument names a `.vyx` file

`main.rs:1371-1391` resolves a generator's first string argument through
`resolve_spec`, whose `with_ext` (`loader.rs:368-374`) appends `.vyrn` to
anything not ending in `.vyrn` or `.json`. A `.vyx` argument becomes
`.../Leak.vyx.vyrn`, matches nothing, and the edge is recorded against a name
nothing else uses. On the A5.1 tree, where the client root demonstrably mounts
the page:

```
$ vyrn why server/pages/Leak.vyx
  audience: server-only — path segment `server` (vyrn.json audience.server)
  imported by: nothing in this project reaches it

$ vyrn why server/store.vyrn
  imported by:
    server/pages/Leak.vyx -> server/store.vyrn
```

The chain stops inside `server/` and the client root never appears. RFC-0072:111-113
makes `vyrn why` the answer to "what is bundled where … a command, not a
convention you have to trust", and RFC-0072:344-346 says generator imports
contribute edges. For per-file `.vyx` mounts they do not. This is independent of
A5.1: closing the checker hole leaves the report wrong.

### A5.10 CONFIRMED — Medium. `//@diag` de-duplication ignores the column, so a second report on one line is hidden
...
Two reports at different columns of one line, with identical text:

```
$ vyrn check app_dedup.vyrn
#   //@diag warning ./data/two_lines.txt:1:3  img has no alt
#   //@diag warning ./data/two_lines.txt:1:40 img has no alt
data/two_lines.txt:1:3: warning: img has no alt
exit=0
```

One survivor. Making the messages differ produces both. This is the direct answer
to "can two locations collapse onto one and hide one": yes. The shape is
RFC-0099 M2's own worked example — `a11y/img-alt` on a template line carrying two
`<img>` tags reports one and swallows the other. RFC-0099:210-212 justifies
de-duplication for a page generated twice (server plus client bundle); keying on
`(file, line, col, message)` serves that case and not this one.

### A5.11 CONFIRMED — Medium. A malformed directive cancels a valid one, and `//@origin` has no delimiter requirement

`origin.rs:99` uses `strip_prefix("//@origin")` with no trailing-delimiter check,
and `origin.rs:153-161` takes the last directive at or before the diagnostic
line, then returns `false` if it is malformed. So a generated comment that merely
begins with the word — `//@originally derived from ..` — blinds the map for every
line below it, and a valid position followed by any trailing text is discarded
whole:

```
$ vyrn check app_extra.vyrn      # //@origin ./data/two_lines.txt:1:1 and some trailing words
generated by emit("extra") at app_extra.vyrn:2:0: ...
  note: malformed `//@origin` directive (bad column `1 and some trailing words`);
        reported at generated location
```

Nothing is lost, so the never-lose rule holds here. The two parsers disagree,
though: `//@diag` splits the anchor at the first whitespace (`origin.rs:303-311`)
and tolerates a trailing message; `//@origin` does not. `//@origin` also accepts a
tab or any non-ASCII inside the path, which breaks the `file:line:col:` shape of
the printed line.

### A5.12 Design critique — Low. Every diagnostic in a region collapses onto the region's start column

`origin.rs:171-173` assigns the directive's line and column and discards the
generated column. RFC-0053:142-148 records the decision with its argument
(generator indentation and wrapping make the offset unrecoverable, so the region
start is the honest answer). The cost is visible: four errors from two distinct
generated lines print at one user position, separated only by their notes, and
`std/ui` emits `file:1:1` region directives for a whole page's dispatch glue
(RFC-0033:127-130), so every router error in a page stacks on line 1 column 1.

### A5.13 CONFIRMED — Low. `roles` gates nothing in the compile path

`contracts::roles_from_manifest` has three callers: `vyrn why --contract`
(`main.rs:771`), the LSP (`vyrn-lsp/src/contracts.rs:113`), and tests. `vyrn
check` never reads the key. With `"roles": {"server/pages": "std/ui:Page"}` and a
module exporting `hedd` instead of `head` — the typo RFC-0072 M2 says is now an
error (lines 386-394):

```
$ vyrn check server/main.vyrn
ok
$ vyrn why --contract server/pages/one.vyrn
  UNKNOWN   hedd: not named by the contract — did you mean `head`?
  — 1 objection(s); the generator that consumes this module is the gate
exit=0
```

The tool states the rule itself: the generator is the gate. A module in a
declared role that no generator consumes is graded by nothing. Consistent with
RFC-0071 M4's "reports, does not gate", so a design critique — but the last line
of that output is the honest summary of what a `roles` declaration buys, and it
is less than the RFC's framing suggests.

### A5.14 Design critique — Low. A segment declared under two audiences resolves server-first

`audience.rs:339-350` returns the first match in server, client, universal order,
with the comment "declaring one twice is a manifest mistake, not a language
question". The outcome is the conservative one — the import is refused — so the
risk is confusion, not leakage. Worth pairing with A5.8: both are cases where the
manifest is read without being checked, and here the stricter reading wins by the
order of an `if` rather than by a rule.

### A5.15 PLAUSIBLE — Low. Two classifier shortcuts that the CLI cannot currently reach

Recorded because A5.2 shows the classifier's string reasoning is where the rule
breaks. `relative_to` (`audience.rs:361-375`) takes a relative key against an
absolute base as already project-relative, so every component of the working
directory would be scanned for audience segments; a project under a directory
literally named `server`, checked from three different working directories,
stayed universal because the CLI canonicalizes the root. `same_path`
(`audience.rs:265-271`) matches an entry point by suffix on a component boundary,
so with an empty base any module named `boot.vyrn` would take the `client`
entry's audience; a manifest always supplies the base. Both become findings if
module keys ever stop being canonicalized — and R3.5 is the case where a key
already escapes the base.

### What this lens found sound

* **Every widening edge written as a path import is refused, in every spelling
  the loader normalizes.** Eight forms were tried against one fixture and all
  eight were refused with the same diagnostic: `import * as ns` (RFC-0027), `{ x
  as y }` co-naming (RFC-0022), an explicit `.vyrn`, `../app/../server/store`,
  `.//..//server//store`, a bare specifier through `vyrn.json:dependencies`, a
  re-export relay through a universal module (reported at the correct end of the
  edge), and a bare `import type`.
* **A well-behaved generator's origin map is column-exact.** `std/vyx` puts a
  `.vyx` error on the right line and the right column, including the second of
  two interpolations on one line (`comp/Card.vyx:7:63`) and inside an event
  handler's argument (`comp/Card.vyx:10:22`).
* **The never-lose rule holds on the CLI for every malformed, out-of-order or
  `end`-terminated directive tried.** The diagnostic keeps its generated location
  and gains a note that says why.
* **The generator cache's absence tracking works as RFC-0021 describes.** An
  input recorded `absent` mismatches every content hash, so a file that appears
  after a generation misses; the `v2` tag rejects an older entry, which is also a
  miss. The soundness gap in that cache is not the key or the record — it is that
  the entry is not authenticated at all (C2.1).
