...
- A background command is the Bash tool's `run_in_background` option, and nothing else: not `&`, not `nohup`, not a marker file you check later. A command started any other way never wakes you, and the only thing left is to poll, which is forbidden. A tool call whose purpose is to pass time (`echo`, `test -f done`, `sleep`) is a poll; three agents on three days have written one, and each was found by the user.
...
- A notification can be lost: the machine kills a background task under memory pressure and says nothing. Before you wait for one, list the processes whose command line names your worktree. If none is alive, nothing is coming: read the log you redirected to and continue. Three tracks stood idle for an hour on 2026-09-11 waiting for builds that had finished.
