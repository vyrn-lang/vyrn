...
- A rebase conflict in a pin (a census `want` table, `SHAPE_PIN`, `wasm-sha256.tsv`) is not resolved by hand. Take the base's side, keep the slice's own rows beside it, and run the gate that prints the pin; the manifest rewrites itself with `VYRN_WASM_MANIFEST=write`. A rebase with no code conflict runs the short list once, at the tip.
...
- A track runs every command in the foreground under `timeout 600`. Every local cargo test runs in the release profile, so a worktree builds one profile and not two, and runs under `cargo nextest`, which runs the test binaries at once. Measured on main on 2026-09-24: the whole CLI suite in 63 s, and `coredrive` in 91 s against 280 s to 550 s in the debug profile. CI keeps the debug profile, so a debug-only failure still reaches a gate. A track never uses the Bash tool's `run_in_background` option: a teammate is not woken when its background command ends (three tracks stood idle for half an hour twice on 2026-09-11, each after a build that had finished in under a minute), and the only thing left is to poll, which is forbidden. Nor `&`, `nohup`, or a marker file: the same silence. A tool call whose purpose is to pass time (`echo`, `test -f done`, `sleep`) is a poll; three agents on three days have written one, and each was found by the user.
...
- If you find yourself waiting for anything, list the processes whose command line names your worktree. If none is alive, nothing is coming: read the log you redirected to and continue.
...
The short list, which every track runs in the foreground at the branch's tip before it reports, one commit's worth of gates and not one per commit: `cargo fmt --all --check`; `cargo fmt --manifest-path vyrn-lsp/Cargo.toml --check`; `cargo build --release -p vyrn-cli` with no new warning; the CLI suite, `cargo nextest run --release -p vyrn-cli --no-fail-fast --status-level fail`; the judgments, the core walk and the manifest, `VYRN_WASM_MANIFEST=check cargo nextest run --release -p vyrn-cli --no-fail-fast --status-level fail --success-output final --run-ignored only -E 'binary(kernel) | binary(effects) | binary(typed) | binary(coretables) | binary(coredrive) | binary(wasmhash)'`; every census the change touched; `vyrn check` over the 415 roots, `scripts/check-corpus.sh <tree> <out>` for the base's tree and the head's, then `diff -r` on the two outputs; when the change touches `std/`, `target/release/vyrn doc --std -o ../docs/api --verify` from `compiler/`, and commit what `vyrn doc` regenerates; when it touches the lexer's reserved words, `node --test "web/test/*.test.mjs" "editor/vscode/test/*.test.mjs"`, because the editor grammar's keyword list must equal the lexer's; the licence of the slice (the corpus diff, the manifest check, the lowering pin, the kernel corpus, or the residue ratchet, whichever the slice's licence names). Each command runs under a timeout with its output in a file.
